...
# Download the raw file by clicking the download button on the GitHub site and put in workind director with your RMarkdown fil
# Import by clicking on "browse" after clicking Import Dataset ->  From Text (readr), and then navigating to the file


library(readr)
msn2_4_mutants_EtOH <- read_delim("msn2-4_mutants_EtOH.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE)
View(msn2_4_mutants_EtOH)

# manually load from URL using Important Dataset -> From Text (readr)
# Note: click on the raw data link in GitHub to get to the URL with the text file

msn2_4_mutants_EtOH <- read_delim("https://raw.githubusercontent.com/clstacy/GenomicDataAnalysis_Fa23/main/data/ethanol_stress/msn2-4_mutants_EtOH.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
This dataset includes the log~2~ fold changes of gene expression in an experiment testing the ethanol (EtOH) stress response for the YPS606 strain of *S. cerevisiae* and an *msn2/4ΔΔ* (gene deletion) mutant. There are also additional columns of metadata about each gene. In later classes, we will cover the details included, but we can already start answering questions.

**Using RStudio, answer the following questions:**

1.  How many genes are included in this study?

2.  Which gene has the highest log~2~ fold change in the *msn2/4ΔΔ* mutant EtOH response?

3.  How many HSP (heat shock protein) genes are differentially expressed (FDR \< 0.01) in unstressed conditions for the mutant?

4.  Do the genes with the largest magnitude fold changes tend to have the smallest p-values?

5.  Which isoform of phosphoglucomutase is upregulated in response to ethanol stress? Do you think *msn2/4* is responsible for this difference? Why or why not?
