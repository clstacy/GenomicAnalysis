...
-   Be able to call peaks using the MACS2 peak caller
-   Identify genes with a trranscriptional start site (TSS) nearest to
    peaks found in ChIP-seq analysis
-   Conduct and interpret enrichment analyses of putative target gene
    lists.

## Exercise Background

In this exercise, we will look at published ChIP-seq data for Msn2
binding during the yeast metabolic cycle (which consists of respiratory
oscillations, with stress gene induction happening at the end of the
cycle). The data comes from <https://elifesciences.org/articles/29938>.
We will go through this analysis together, starting from the raw FASTQ
files.
...
likely won't run for you (so run those commands in that exercise first
if this next chunk does not work for you).

##### MACS2

A popular software for peak-calling ChIP-seq data is
[MACS2](https://pypi.org/project/MACS2/). It uses a Poisson
distribution-based model of the background to identify significant peaks
in ChIP-seq data.
...
For today's exercise, the software Model-based Analysis of ChIP-seq
(MACS) will be doing the ChIP-seq specific part of our analysis. MACS
needs .bam files as input, which we can generate in the same way as we
did in the 04_Read_Mapping.Rmd exercise. However, we get .fastq files
from sequencing, so we have to align the fastq files to the genome. A
supplement
...
ChIPseq will often show the presence of common artifacts, such as
ultra-high signal regions. Such regions can confound peak calling,
fragment length estimation and QC metrics. Anshul Kundaje created the
DAC blacklist as a reference to help deal with these regions. These are
available for dm3, dm6, ce10, ce11, mm10, hg19, and hg38.
(<https://zenodo.org/records/1491733>).

*Saccharomyces cerevisiae* doesn't have an official blacklist, so we
won't be using this today.

### QC & Trimming

We can use **FastQC** to do quality control for fastq files generated in
ChIP-seq experiments. For the sake of time in class, we will skip using
FastQC on the reads today. We can look at the rfastp QC reports in
...
We can use rfastp to trim and QC all of our files, by slightly modifying
the original rfastp command we used for rnaseq data. The only major
modification is including the read2 in rfastp. QC reports and trimmed
fastq files will appear in the dir_trimmed.fq_files folder.
...
Assign the read1 trimmed fastq files to a list in R called
trimmed_fastq_files_R1, and we'll do the same for read2 fastq files.
...
We have already done this in the Read_mapping.Rmd exercise. If you've
already created this, we can skip this step.

```{r}
# Uncomment if the next code chunk doesn't work, but you should already have created the index during read mapping
...
In this analysis, we only need to do a single call to MACS2 because we
only have one treatment sample and one control sample. If we had more
samples, we'd want to run MACS2 for every sample.

For the purpose of demonstration, we will run MACS2 on each sample with
no control, because those are the data we have. NOTE: THIS IS NOT
RECOMMENDED. For this analysis, we should just use the third call to
MACS2 below. We are running the calling without controls so we can look
at the peaks that would be called for each alone, so we can compare them
to the peaks of the treatment vs control.
...
We can use R to analyze the peaks generated by MACS2. There are many
approaches for doing this. We will use the `ChIPSeeker` package,
followed by tools we've previously used for downstream analysis.

### Importing Peaks

MACS peak calls can be found in our specified output directory with the
suffix and extension "\_peaks.xls". MACS peaks come as a tab separated
file (thinly disguised as a ".xls"). In addition to the genomic
coordinates of peaks, these files contain useful information on the
samples, parameters and version used for peak calling at the top.
...
No peaks were detected in the blacklisted region, so none are filtered
out. If you have blacklist regions in your genome, you can remove them
now. A variety of software tools are available for excluding them
automatically, including bedtools intersect.
...
Currently, we have peaks loaded in as a data.frame object
`MSN24_ESR_t20vst0_peaks`. Note that none of the columns in that file
tell us which gene(s) each peak corresponds to. In order to do annotate
the peaks, we first need to convert out data frame to a GRanges object
...
Next, we need to load the TxDb for *S. cerevisiae*. The
`GenomicFeatures` package uses TxDb objects to store transcript
metadata. This contains information about the 5' and 3' untranslated
regions (UTRs), protein coding sequences (CDSs) and exons for a set of
mRNA transcripts to their associated genome. TxDb objects have numerous
accessory functions to allow such features to be retrieved individually
or grouped together in a way that reflects the underlying biology.

A list of currently available TxDb's is available at
...
Now that we have a GRanges object and our TxDb object, we can use the
`annotatePeak` function from the `ChIPseeker` package to create an
object containing the annotated peaks
...
The `ChIPSeeker` package in R contains some functions for visualizing
the annotated peaks. We can look at some here.

We see from this pie chart that most of the peaks occurred within 1kb of
the TSS for the corresponding gene, with less than 3% of peaks further
away.
...
However, we know many features overlap, so we can use a vennpie instead.
This shows us that of our peaks extend into regions that contain genes.
...
Institute. You may already have this downloaded on your laptop.
Alternatively, we can use the web-based IGV to visualize the data. We
will look at bedgraph files (which are similar to bigwig files) that
include a column for read density.

Go to <https://igv.org/app/>, select genome as sacCer3, and upload file
via Tracks \> local file..., and select the
MSN24_ESR_t20vst0_treat_pileup.bedgraph (by default in
\~/Desktop/Genomic_Data_Analysis/Analysis/ChIP). You can also upload the
.bed file in the same folder.

## Downstream Analysis

ChIP peaks in R: Let's generate a gene list corresponding to peaks that
appear in t20 minutes but not t0 minutes. We already have the peaks
annotated to genes, so we just need to get a list of genes.
...
Now we have a list of genes that correspond to the peaks seen in
treatment but not control. Let's see if there are any enrichments in
this gene list, using techniques we have used already in class.
...
specific algorithm for ChIP-seq analysis, called MEME-ChIP. This
workflow hasn't been implemented in the `memes` package in R yet, but
can be done in the website at
<https://meme-suite.org/meme/tools/meme-chip>. You can upload a fasta
file of the peak summits to be analyzed.

First, let's extract the sequences corresponding to the annotated peak
ranges. We will use the `get_sequence` function from the `memes` package
for this. This function requires a BSgenome database to pull sequences
from, so it has been loaded in.
...
We can use Streme to look for enriched motifs in the promoters of those
genes corresponding to enrichment peaks. Once Streme finishes, we can
check out the .html output file in the output folder.
...
Finally, use TomTom to see which TFs those motifs might correspond to.
We can check the .html output file for TomTom as well.
...
1.  Based on the results obtained from the MACS2 analysis, what can you
    infer about the genomic regions where Msn2 is binding?

2.  If you were designing a ChIP-seq experiment to study Msn2 binding
    under different conditions, what factors would you consider in your
    experimental design

3.  What can ChIP-seq tell us about Msn2 that our previous analyses
    cannot?
