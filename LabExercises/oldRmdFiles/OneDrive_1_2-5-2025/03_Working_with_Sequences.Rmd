...
We see all of the nucleotides are assigned to A, C, G, or T. N is any nucleotide (used when the base caller has no confidence at all in the call, Phred = 2 or lower). The other letters are IUPAC codes for ambiguous bases (e.g., M = "A" or "C). IUPAC codes have been replaced by Phred scores.

A fundamental difference between FASTA and FASTQ files is the quality scores contained in FASTQ.

Quality scores are stored as ASCII characters representing -log10 probability of a call base being wrong (higher scores mean higher confidence in the accuracy of the base calls).
...
One very useful function is the `alphabetByCycle()` function which provides a quick method to summarize base occurrence of cycles.
...
The `qa()` function accepts a FASTQ file and returns a FASTQA object.
