...
## If you are unable to download directly from github using this script, you can download salmon.gene_counts.merged.nonsubsamp.tsv from OneDrive, and then import the dataset (From Text (readr)) as a tab-delimited text file. Then uncomment and use the following code:

## counts <- salmon_gene_counts_merged_nonsubsamp
## counts = column_to_rownames(counts2, "Name")
```
...
As described for the edgeR module, genes with very low counts across
samples should be filtered out because they add little if anything to
the analysis and they increase the multiple testing burden. In this
case, we will retain a gene only if it is expressed at a
count-per-million (CPM) above 60 in at least four samples.
...
While limma analysis does not strictly require TMM normalization, it is
generally recommended to be performed to eliminate composition biases
between libraries. This is done with edgeR and generates a set of
normalization factors, where the product of these factors and the
library sizes defines the effective library size. The calcNormFactors
function returns the DGEList argument with only the norm.factors
changed.
...
workflow, where voom is being used to estimate the mean-variance
relationships for each gene instead of using the negative binomial
distribution of edgeR.
...
Limma uses the `lmFit` function to fit a linear model to each gene. This
returns a MArrayLM object containing the weighted least squares
...
expression for each gene, using empirical Bayes moderation of the
standard errors for each gene towards a global value. The contrast of
interest can be specified using the `makeContrasts` function in limma,
...
Question 3: What are the differences in genes being called
differentially expressed for edgeR, limma, and DESeq2?

Question 4: Are certain functional categories (GO enrichments) more
likely to show up uniquely using certain tools?

Question 5: What are the logFC and adjusted p-values for genes that show
up uniquely using certain tools?
