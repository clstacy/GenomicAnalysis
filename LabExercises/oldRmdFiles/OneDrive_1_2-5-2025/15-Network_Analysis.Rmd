...
Use WGCNA to identify modules identified in patterns of transcriptomic changes across samples spanning multiple genotypes and stress conditions. Through this exercise, we aim to gain practical insights into interpreting WGCNA outputs and leveraging this technique for a deeper understanding of genomic landscapes and network analysis.
...
Raw counts need to be normalized for read depth to be comparable across samples. Let's use edgeR to normalize the raw counts by converting them to cpm. Also, WGCNA cannot handle missing data, so transcripts with zero counts in any sample are removed. 
...
Now that we've normalized the cpm data, let's look at the distributions again
...
For WGCNA, we don't want to correlate all of the genes because (1) computational limitation and (2) genes that vary a small amount increase the number of variables in the model, and are uninformative, making it harder to be confident in the patterns we do see. One way to account for this is by subsetting our entire gene list. 

One common approach for subsetting genes is taking genes whose variance is greater than the $n^{th}$ quantile of gene variances. We'll do that now:
...
There's significant debate on whether or not to filter on variance (hub genes may not vary much), so we will use light filtering on the 10% of genes with the lowest variance

```{r find-quantile-rowwise-variances-wgcna}
# get variance corresponding to 10th percentile
q10_wpn <- quantile( matrixStats::rowVars(normalized_counts), .1)  # common cutoff

# # get variance corresponding to 95th percentile--see how the data changes with high filtering
...
normalized_counts_variable_genes <- normalized_counts[ rv_wpn > q10_wpn, ]

str(normalized_counts_variable_genes)
```
Notice the number of genes retained is a bit fewer.
...
mergedColors_var_genes = variable_genes_network$colors
...
modules_variable_genes_df <- data.frame(
  ...
  colors = variable_genes_network$colors
)
...
```{r}
# Convert labels to colors for plotting
mergedColors_DE = DE_genes_network$colors
# Plot the dendrogram and the module colors underneath
plotDendroAndColors(
...
modules_DE_genes_df <- data.frame(
  gene_id = names(DE_genes_network$colors),
  colors = DE_genes_network$colors
)
...
Question 4: What does module preservation in WGCNA mean, and how does the model generated with just differentially-expressed gens compare to the one where only veyr low variance genes were removed?

Question 5: Do you think it's apporpriate for a scale-free network to filter genes based on whether they are differentially expressed?
