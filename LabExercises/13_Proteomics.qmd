---
title: "Proteomics Analysis with limma"
...
-   Recognize the steps of differential protein expression analysis of Tandem Mass Tag (TMT) mass spectrometry data.
-   Use `limma` to identify differentially expressed proteins.
-   Better understand the temporal relationship between changes in RNA abundance and protein abundance.

## Exercise Background

In this exercise, we will compare transcriptomic and proteomic time course data for the yeast heat shock response (shift from cell growing at 25°C to 37°C). The proteomic data is median centered log2 protein counts from this paper: <https://pubs.acs.org/doi/10.1021/acs.jproteome.9b00704>. We will compare the dynamics of protein abundance changes to RNA changes using microarray data from this paper:
<https://academic.oup.com/genetics/article/184/2/587/6077881>. Those data are median centered log2 ratios of each heat shock time point compared to an unstressed control.
...
pak(c(
  ...
  "dplyr", "stringr", "statmod", "Glimma", "purrr", "reactable"
  )
)

pak(c("edgeR", "AnnotationDbi", "org.Sc.sgd.db", "ggVennDiagram","janitor","RColorBrewer"
,"UpSetR","ComplexHeatmap","enrichplot","clusterProfiler","factoextra", "NbClust","ggpubr", "corrplot", "stringr", "tidyr")
)
```
...
*We are downloading the normalized counts for the TMT proteomics data from a Github repository using the code below, and assigning the data to the variable `proteome_HS`.*

```{r load-proteome-raw, error=TRUE}
proteome_HS <- read.delim("https://github.com/clstacy/GenomicAnalysis/raw/main/Data/heat_shock/proteomic_FC/HS_Proteomics_Data.txt",
                          sep="\t",
                          header=T,
                          row.names=1)
```
...
colnames(design_proteome) <- colnames(design_proteome) |> stringr::str_remove("_proteome")
```
...
kegg_results_proteome@result$Description <- kegg_results_proteome@result$Description |>
  print() |>
  str_replace_all(string=_, fixed(" - Saccharomyces cerevisiae"), "")
...
ID_proteins_list <- results_proteome |>
  ...
  filter(DE_direction != 0) |>
  # split into many lists
  group_split() |> 
  (\(x) purrr::set_names(x, purrr::map_chr(x, ~ .x$group[1])))() |>
  purrr::map(~ pull(.x, all_of(ORF)))

# See the resulting list
glimpse(ID_proteins_list)
```
...
heatmap_protein <- res_all_proteome_matrix |>
  ...
  dplyr::filter(ORF %in% purrr::list_c(ID_proteins_list)) |>
...
```{r}
transcriptome_HS <- read.delim("https://github.com/clstacy/GenomicAnalysis/raw/main/Data/heat_shock/Eng_2010_HS_Arrays.tsv",
                          sep="\t",
                          header=T,
                          row.names=1,
                          check.names = F) |>
...
ID_transcriptome_list <- c(purrr::list_c(ID_transcriptome_list_up), purrr::list_c(ID_transcriptome_list_down))
...
perform_correlation_analysis(
  res_all_proteome,
  res_all_transcriptome,
  purrr::list_c(ID_transcriptome_list_up),
  "upregulated",
...
perform_correlation_analysis(
  res_all_proteome,
  res_all_transcriptome,
  purrr::list_c(ID_transcriptome_list_down),
  "downregulated",
