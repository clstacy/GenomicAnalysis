...
-   **Using R Packages:** load and utilize key bioinformatics
...
pak(c("biomaRt", "memes", "Biostrings", "curl", "universalmotif")
   )
...
in the genome. In yeast, this isn't too much data for a
...
widely used. However, it is not always the best approach. 

MEME is recommended when you have fewer than 50 sequences, while STREME is
...
motifs in public databases. The TomTom algorithm, part of the MEME-suite,
allows us to do that, comparing our identified motifs to known motifs
in annotation databases.

First, we need to download the YEASTRACT database file called
(YEASTRACT_20130918.meme). We pull this file from Github, but you
