...
The following code installs all of the packages used in this document -- if not already installed -- and then loads the packages into R. We need to install packages specific to our gene ontology bioinformatic analysis. Many of these packages aren't available on the R CRAN package repository, instead they are hosted on the BioConductor repository, a repository that is focused on packages used in biological research. Today, we need to install the package clusterProfiler with the code below. The `pak()` function will check the bioconductor repository if the package isn't on CRAN
...
pak(c("tidyverse", "here", "knitr", "dplyr", # already downloaded last activity
      ...
      # "igraph", 
       "tidytree", # dependencies that require explicit download on latest Mac OS
...
We need a list of differentially expressed genes to test for over (or under) enrichment of functional GO terms. Here, we choose genes with significantly (FDR\<0.05) higher expression (log2 fold change (logFC) greater than 1) in the *msn2/4ΔΔ* mutant's EtOH response compared to the EtOH response of the wild-type strain (positive values in the logFC column of WT vs *msn2/4ΔΔ*: EtOH response).

```{r}
# subset data to just genes with significant fdr & log2FC>1
msn24_EtOH |>
  filter(logFC_WT.vs.msn24dd_EtOH_response > 1 & 
           fdr_WT.vs.msn24dd_EtOH_response < 0.05)
```

```{r}
#  Save results to a new variable:
DE_genes_upregulated_msn24_EtOH <- msn24_EtOH |>
  filter(logFC_WT.vs.msn24dd_EtOH_response > 1 & 
         fdr_WT.vs.msn24dd_EtOH_response < 0.05) |>
...
#|   - "GO Enrichment of upregulated genes in *msn2/4ΔΔ* EtOH response"
...
ggsave(filename = "GO_BP_msn24_EtOH_up_results_lollipopPlot.pdf", 
  plot = last_plot(), 
...
phyper(q = Overlap-1, # number of integration genes that were DE
       ...
       k = leucine_genes, # number of observed DE integration genes
...
phyper(q = Overlap, # number of integration genes that were DE
       m = DE_genes, # number of DE genes
       n = total-DE_genes, # number of non DE genes
       k = leucine_genes, # number of observed DE DNA integration genes
...
fisher.test(matrix(c(Overlap, DE_genes-Overlap, leucine_genes-Overlap, total-DE_genes-leucine_genes + Overlap), 2, 2), alternative='greater')$p.value

#fisher test for underrepresentation
fisher.test(matrix(c(Overlap, DE_genes-Overlap, leucine_genes-Overlap, total-DE_genes-leucine_genes + Overlap), 2, 2), alternative='less')$p.value
```
...
-   See what happens when you change the FDR threshold from a less stringet one (0.05) to a more conservative one (0.01).
...
ggVennDiagram(GO_results_list,
              category.names = c("msn24 EtOH upregulated", "NAME ME")) +
