...
In this exercise, we will look at a published ChIP-seq data for
Msn2/4 mutants under EtOH stress conditions similar to those we have
been exploring throughout this course. The data comes from SRA accession SRP033438, from the publication [<https://doi.org/10.1093/nar/gku176>]. We will
...
Similar to exercise `03_WorkingWithSequences.qmd`, we need to use software that is not available in R. To install these software, we can create and activate conda environments. 

**If you haven't done set-up of conda on your system as described in Exercise 03, this code likely won't run for you.**

#### MACS2

A popular software for peak-calling ChIP-seq data is
[MACS2](https://pypi.org/project/MACS2/). It uses a Poisson
distribution-based model to background identify significant peaks in ChIP-seq data.

The code chunk below creates a conda environment called `MACS` and installs MACS2 in that environment.
...
# libmamba is a faster solver for conda
#conda install -n base conda-libmamba-solver
#conda config --set solver libmamba

# create an enviornment for our QC packages
if conda info --envs | grep -q MACS; then echo "environment 'MACS' already exists"; else conda create --experimental-solver=libmamba -y -n MACS macs2 -c bioconda; fi
...
# good coding practice is to deactivate a conda environment
...
For today's exercise, the software Model-based Analysis of ChIP-seq (MACS) will be doing the ChIP-seq specific part of our analysis. MACS needs `.bam` files as input, which we can generate in the same way as we did in the `04_ReadMapping.qmd` exercise. 
However, we get `.fastq` files from sequencing, so we have to align the fastq files to the genome to generate `.bam` files.

### FASTQ files

We will be analyzing paired-end FASTQ files from the publication <https://doi.org/10.1093/nar/gku176>, with experiment SRA accession SRP033438. 
Specifically, we will be comparing data from time 0 minutes (SRR1181480) and time 20 minutes (SRR1181481) post glucose to glycerol shift, an environment induces an environmental stress response in yeast. 
...
## Using control sample here is best practice
macs2 callpeak -t "/Users/$USER/Desktop/Genomic_Data_Analysis/Data/Bam/ChIP/SRR1181481_R1.fastq.gz.subread.BAM" \
               -n MSN24_ESR_t20vst0 \
...
MACS peak calls can be found in our specified output directory with the ending "_peaks.xls".
