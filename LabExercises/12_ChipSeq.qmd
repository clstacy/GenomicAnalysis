...
In this exercise, we will analyze ChIP-seq data.
...
-   Be able to call peaks using macs3 peak caller
...
In this exercise, we will look at a published ChIP-seq data for Msn2/4 mutants under EtOH stress conditions similar to those we have been exploring throughout this course. The data comes from SRA accession SRP033438, from the publication \[<https://doi.org/10.1093/nar/gku176>\]. We will go through this analysis together, starting from the raw fastq files.

## Pre-Analysis Preparations

Before analyzing our ChIP-seq results, we need to prepare our computing environment for this analysis.
...
pak(c(
  "dplyr", "stringr", "Rfastp", "Rsubread",
  "ChIPseeker", "GenomicRanges", #"Iranges",
  "BiocManager", "ggimage", "ggupset",
  "TxDb.Scerevisiae.UCSC.sacCer3.sgdGene",
  "BSgenome.Scerevisiae.UCSC.sacCer3",
  "clusterProfiler", "forcats", "memes", "curl",
  "MACSr" # for the MACS software
  )
)
...
A popular software for peak-calling ChIP-seq data is [MACS3](https://macs3-project.github.io/MACS/). It uses a Poisson distribution-based model to background identify significant peaks in ChIP-seq data.

We are using an R package wrapper for this tool, called `MACSr`


## Data Retreival

For today's exercise, the software Model-based Analysis of ChIP-seq (MACS) will be doing the ChIP-seq specific part of our analysis. MACS needs `.bam` files as input, which we can generate in the same way as we did in the `04_ReadMapping.qmd` exercise. However, we get `.fastq` files from sequencing, so we have to align the fastq files to the genome to generate `.bam` files.

### FASTQ files

We will be analyzing paired-end FASTQ files from the publication <https://doi.org/10.1093/nar/gku176>, with experiment SRA accession SRP033438. Specifically, we will be comparing data from time 0 minutes (SRR1181480) and time 20 minutes (SRR1181481) post glucose to glycerol shift, an environment that induces an environmental stress response in yeast.

#### Two options for downloading the data:

We could use the SRA toolkit to download these files (tutorial [here](https://erilu.github.io/python-fastq-downloader/#downloading-fastq-files-using-the-sra-toolkit)).

An alternative approach we can use to download these files is going to <https://www.ebi.ac.uk/ena/browser/view/SRR1181480> and <https://www.ebi.ac.uk/ena/browser/view/SRR1181481> to download the files manually, or download the bash script (`Get download script`) that we can use to download the files. We will use a modified version of that code to download the files today.

```{bash fetch-fastq-chipseq, engine.opts='-l'}
...
*Saccharomyces cerevisiae* doesn't have an official blacklist, so we won't be using a blacklist in today's activity.
...
We can use `rfastp` to trim and QC all of our files, by slightly modifying the original `rfastp` command we used for RNAseq data. The only major change is including the `read2=` in `rfastp`. QC reports and trimmed fastq files will appear in the `dir_trimmed.fq_files` folder.
...
for (i in seq_along(fq_read1_files)) {
...
Now, we are able to call peaks with macs3.

To run macs3 to we need to supply:

```         
1. A BAM file to find enriched regions in. (tfile)
  a. this is the only REQUIRED parameter for MACS
2. A Name for peak calls (name).
3. An output folder to write peaks into (outdir).
4. Optionally, but highly recommended, we can identify a control to compare to (cfile).
```

In this analysis, we only need to do a single call to macs3 because we only have one treatment sample and one control sample. If we had more samples, we'd want to run macs3 for every sample.

For the purpose of demonstration, we will run macs3 on each sample with no control. **THIS IS NOT RECOMMENDED**. For this analysis, we should just use the third call to macs3 below. We are running the calling without controls so we can look at the peaks that would be called for each alone, so we can compare them to the peaks of the treatment vs control.

Flags we've used for MACS3:

```         
- `name` output file name prefix
- `outdir` path to where output files should be saved
- `format` type of files being input. Usually `BAM` (or `BAMPE` for paired-end.)
- `gsize` is the genome size. this has been set for yeast genome
- `store_bdg` output bedgraph files
- `nomodel` MACS will bypass building the shifting model.
- `extsize` MACS uses this parameter to extend reads in 5'->3' direction to fix-sized fragments.
```


```{r run-MACS3}
macs_outdir <- path.expand("~/Desktop/Genomic_Data_Analysis/Analysis/ChIP/")

# create macs_outdir if it doesn't exist
if (!dir.exists(macs_outdir)) {
  dir.create(macs_outdir, recursive = TRUE)
}

# run macs on the 0 minutes sample, no control
callpeak_0min_noControl <- callpeak(
  tfile = path.expand("~/Desktop/Genomic_Data_Analysis/Data/Bam/ChIP/SRR1181480_R1.fastq.gz.subread.BAM"),
  name = "MSN24_ESR_t0_noControl",
  outdir = macs_outdir,
  format = "BAMPE",
  gsize = "1.2e7",
  store_bdg = TRUE,
...
callpeak_20min_noControl <- callpeak(
  tfile = path.expand("~/Desktop/Genomic_Data_Analysis/Data/Bam/ChIP/SRR1181481_R1.fastq.gz.subread.BAM"),
  name = "MSN24_ESR_t20_noControl",
  outdir = macs_outdir,
  format = "BAMPE",
  gsize = "1.2e7",
  store_bdg = TRUE,
...
## Using a control sample here is best practice
callpeak_20v0min <- callpeak(
  tfile = path.expand("~/Desktop/Genomic_Data_Analysis/Data/Bam/ChIP/SRR1181481_R1.fastq.gz.subread.BAM"),
  cfile= path.expand("~/Desktop/Genomic_Data_Analysis/Data/Bam/ChIP/SRR1181480_R1.fastq.gz.subread.BAM"),
  name = "MSN24_ESR_t20vst0",
  outdir = macs_outdir,
  format = "BAMPE",
  gsize = "1.2e7",
  store_bdg = TRUE,
...
```{bash rename-bdg, engine.opts='-l'}
# save output directory to a variable for bash command
...
We can use R to analyze the peaks generated by macs3. There are many approaches for doing this. We will use the `ChIPseeker` package, followed by tools we've previously used for downstream analysis.

## Importing Peaks

MACS peak calls can be found in our specified output directory with the ending "\_peaks.xls". MACS peaks come as a tab separated file (thinly disguised as a ".xls"). In addition to the genomic coordinates of peaks, these files contain useful information on the samples, parameters and version used for peak calling at the top.
...
MSN24_ESR_t20vst0_peaks_filtered <- MSN24_ESR_t20vst0_peaks %>%
  dplyr::filter(chr != "XII" | end <= 451787 | start >= 468932)
```
...
A list of currently available TxDb's is available at https://bioconductor.org/packages/release/BiocViews.html#\_\_\_TxDb

```{r}
# Save the TxDb object for S. cerevisiae to a variable
...
However, we know many features overlap, so we can use a `vennpie` instead. This shows us that many of of our peaks extend into regions that contain genes.
...
Another method for assessing the quality of your alignment is to visualize the alignment using a genome browser. For this workshop we will be using the Integrative Genomics Viewer (IGV) from the Broad Institute. You may already have this downloaded on your computer Alternatively, we can use the web-based IGV to visualize the data.

Go to https://igv.org/app/, select genome as sacCer3, and upload file via `Tracks > local file...`, and select the `MSN24_ESR_t20vst0_treat_pileup.bedgraph` (by default saved to `~/Desktop/Genomic_Data_Analysis/Analysis/ChIP`). You can also upload the .bed file in the same folder.
...
ggplot(go_results,
       showCategory = 15,
       aes(richFactor, forcats::fct_reorder(Description, richFactor))) +
...
The MEME-suite that we used in a previous motif analysis exercise has a specific algorithm for ChIP-seq analysis, called MEME-ChIP. This workflow hasn't been implemented in the `memes` package in R yet, but can be done in the website at https://meme-suite.org/meme/tools/meme-chip. You can upload a fasta file of the peak summits to be analyzed.
...
We can use Streme to look for enriched motifs in the promoters of those genes corresponding to enrichment peaks. Once Streme finishes, we can check out the .html output file in the output folder.
...
if (!file.exists(db_destination)) {
  # URL to download the YEASTRACT database
URL_to_download_YEASTRACT <-
  "https://github.com/clstacy/GenomicAnalysis/raw/main/Reference/YEASTRACT_20130918.meme"
# Choose desired destination for the reference database file
db_destination_YEASTRACT <-
  path.expand("~/Desktop/Genomic_Data_Analysis/Reference/YEASTRACT_20130918.meme")
# Download the file and save to db_destination_YEASTRACT
curl::curl_download(
  url = URL_to_download_YEASTRACT,
  destfile = db_destination_YEASTRACT,
  quiet = FALSE,
  mode = "wb"
)
}
...
View(streme_tomtom_msn24_ChIP)
```

# Questions

1.  Based on the results obtained from the macs3 analysis, what can you infer about the genomic regions where Msn2 is binding?
